...
open import Prelude.Set' as S hiding (_♯_)
open import Prelude.Map' as M hiding (_↦_)
open import Prelude.Lists hiding (dom; codom; _↦_)
...
instance
  ⟦Tx⟧ : Denotable Tx
  ⟦Tx⟧ .⟦_⟧ (p₁ —→⟨ n ⟩ p₂) s = update (update s p₁ (_-ℤ n)) p₂ (_+ℤ n)
...
_↦- : Part → Predˢ
(p ↦-) s = p ∈′ dom s
-- conservative: (_≡ singleton p) ∘ dom

_↦_ : Part → ℤ → Predˢ
(p ↦ n) s = ∃ ((_≡ n) ∘ lookup {k = p} s)
-- Σ[ p∈ ∈ (p ∈' s) ] (lookup {k = p} s p∈ ≡ n)
-- conservative: Σ[ P ∈ (dom s ≡ singleton p) ] (lookup {k = p} s (subst (p ∈′_) (sym P) (here refl)) ≡ n)

_♯_ : S → S → Set
(ps , _) ♯ (ps′ , _) = ps S.♯ ps′

_♯_←_ : S → S → S → Set
s₁ ♯ s₂ ← s = (s₁ ♯ s₂) × (s ≡ᵐ union s₁ s₂)

_∗_ : Predˢ → Predˢ → Predˢ
(P ∗ Q) s = ∃ λ s₁ → ∃ λ s₂ → (s₁ ♯ s₂ ← s) × P s₁ × Q s₂

_—∗_ : Predˢ → Predˢ → Predˢ
(P —∗ Q) s = ∀ s′ → s′ ♯ s → P s′ → Q (union s s′)
...
⟦⟧-♯ :
    ...
    -----------------------------
  → ⟦ l ⟧ s₁ ♯ ⟦ l ⟧ s₂ ← ⟦ l ⟧ s
⟦⟧-♯ {s₁ = s₁}{s₂}{s}{l} (s₁♯s₂ , s₁⊎s₂) = h₁ , h₂
  where
    h₁ : ⟦ l ⟧ s₁ ♯ ⟦ l ⟧ s₂
    h₁ rewrite ⟦⟧-const {s₁}{l} | ⟦⟧-const {s₂}{l} = s₁♯s₂
...
update-empty {s}{t ∷ l} refl rewrite update-emptyᵗ {s}{t} refl = {!!} -- rewrite update-empty {⟦ t ⟧ s}{l} (update-emptyᵗ {s}{t} refl) = refl
